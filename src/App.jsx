...
const App = () => {
  ...
  // API endpoint for shared game state
  const API_URL = 'https://api.jsonbin.io/v3/b';
  const API_KEY = '$2a$10$6W8H5kzLxLqKqVF4X4hxO.6pMxH0yqYvJ0QZxqPZnFHJxXKqYqYqY'; // Public demo key

  // Initialize game board with buildings
  const initializeBoard = () => {
    const board = Array(10).fill(null).map(() => Array(15).fill(null));
    
    // Place buildings (5 of each type, numbered 1-5)
  ...
  // Simple API calls using JSONBin.io (free tier)
  const saveGameToCloud = async (code, data) => {
    try {
      ...
      if (binId) {
        ...
        const response = await fetch(API_URL, {
          ...
          headers: {
            'Content-Type': 'application/json',
            'X-Master-Key': API_KEY,
            'X-Bin-Name': `wacky-${code}`
          },
          body: JSON.stringify(data)
        });
        const result = await response.json();
        localStorage.setItem(`bin-${code}`, result.metadata.id);
      }
    } catch (err) {
  ...
  const loadGameFromCloud = async (code) => {
    try {
      const binId = localStorage.getItem(`bin-${code}`);
  ...
  const createGame = async () => {
    ...
    await saveGameToCloud(newGameId, newGame);
    setGameId(newGameId);
  ...
  const joinGame = async () => {
    ...
    const game = await loadGameFromCloud(joinCode.toUpperCase());
    ...
    await saveGameToCloud(joinCode.toUpperCase(), game);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const updated = await loadGameFromCloud(gameId);
      ...
      <div className="min-h-screen bg-amber-50 p-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            <h1 className="text-4xl font-bold text-amber-900 mb-2">Wacky Wacky West</h1>
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={createGame}
                disabled={loading}
                className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 disabled:opacity-50"
              >
                <Play size={20} />
                {loading ? 'Creating...' : 'Create New Game'}
              </button>
              
              <div className="text-center text-amber-700">or</div>
              
              {!showJoinInput ? (
                <button
                  onClick={() => setShowJoinInput(true)}
                  className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2"
                >
                  <Users size={20} />
                  Join Existing Game
                </button>
            ...
            <div className="mt-8 text-sm text-amber-700 space-y-2 bg-amber-50 p-4 rounded">
              ...
              <ul className="list-disc list-inside space-y-1">
                <li>Each player gets a secret building type to protect</li>
                <li>Place railroad, river, and street tiles across the board</li>
                <li>Tiles destroy buildings they cover - protect yours, demolish rivals!</li>
                <li>Vote when someone tries to demolish an outhouse</li>
                <li>Highest total value of your secret buildings wins!</li>
              </ul>
            </div>
          </div>
        </div>
  ...
  if (gameState.phase === 'waiting') {
    return (
      <div className="min-h-screen bg-amber-50 p-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            <h1 className="text-3xl font-bold text-amber-900 mb-4">Game Lobby</h1>
            
            <div className="bg-amber-100 border-2 border-amber-500 rounded-lg p-4 mb-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-amber-700">Share this code with friends</p>
                ...
                {gameState.players.map(player => (
                  <div
                    ...
                    className={`p-3 rounded ${
                      player.id === playerId ? 'bg-amber-200' : 'bg-gray-100'
                    }`}
                  >
            ...
            {gameState.players.length >= 2 && gameState.players[0].id === playerId && (
              <button
                onClick={startGame}
                disabled={loading}
                className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2 disabled:opacity-50"
              >
                <Play size={20} />
  ...
  if (gameState.phase === 'ended') {
    ...
    return (
      <div className="min-h-screen bg-amber-50 p-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            <h1 className="text-3xl font-bold text-amber-900 mb-6 text-center">🏆 Game Over! 🏆</h1>
            ...
            <button
              ...
              className="w-full mt-6 bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-4 rounded flex items-center justify-center gap-2"
            >
              <RefreshCw size={20} />
              Play Again
            </button>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-4 mb-4">
          <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
            <h1 className="text-2xl font-bold text-amber-900">Wacky Wacky West</h1>
            <div className="text-sm">
              Game: <span className="font-bold text-amber-900">{gameId}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap mb-4">
            {gameState.players.map((player, idx) => (
              ...
              >
                ...
                {idx === gameState.currentPlayerIndex && ' - Your Turn!'}
              </div>
            ))}
          </div>
          ...
          {gameState.phase === 'voting' && (
            <div className="bg-red-50 border-2 border-red-400 p-4 rounded">
              <div className="font-bold text-red-800 mb-2 flex items-center gap-2">
                <Vote size={20} />
                Outhouse Under Threat! Vote Now!
              </div>
              ...
              {gameState.votes.some(v => v.playerId === playerId) && (
                <div className="text-green-700 font-medium">✓ You voted! Waiting for others...</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                      ...
                      {row.map((cell, colIdx) => {
                        ...
                        return (
                          <div
                            ...
                            className={`relative w-12 h-12 border border-amber-300 flex items-center justify-center text-xs ${
                          ...
                          >
                            {cell?.type === 'building' && (
                              <div className="text-center">
                                ...
                                />
                                <div className="text-xs font-bold mt-1">{cell.value}</div>
                              </div>
                            )}
                            {cell?.type === 'outhouse' && (
                            ...
                            {worker && (
                              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white border-2 border-black rounded-full w-8 h-8 flex items-center justify-center text-lg">
                                👷
                              </div>
                            )}
                          </div>
          ...
          <div className="space-y-4">
            ...
            {myPlayer && (
              <div className="bg-white rounded-lg shadow-lg p-4">
                <h3 className="font-bold text-amber-900 mb-2 text-sm">Your Secret Building</h3>
                <div
              ...
              <div className="bg-white rounded-lg shadow-lg p-4">
                ...
                {selectedTile && (
                  <div className="mt-3 p-2 bg-amber-100 rounded text-xs text-amber-900">
                    Click an adjacent square to the {selectedTile.type} worker to place
                  </div>
                )}
              </div>
            )}
            ...
            <div className="bg-amber-50 rounded-lg p-3 text-xs text-amber-900">
              <p className="font-bold mb-1">Quick Rules:</p>
              <ul className="space-y-1 list-disc list-inside">
                <li>Select a tile from your hand</li>
                <li>Place next to matching worker (👷)</li>
                <li>Destroy rival buildings!</li>
                <li>Vote on outhouse demolitions</li>
              </ul>
            </div>
          </div>
        </div>
